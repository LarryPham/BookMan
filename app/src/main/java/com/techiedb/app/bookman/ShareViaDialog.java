    ...
    private void setDisplayList(){
        ...
        for (int i = 0; i< mShareAppList.size(); i++){
            if (!isExistedApp(mDisplayAppList, mShareAppList.get(i))){
                Log.d(TAG, String.format("setDisplayList(): packageName: %s" + mShareAppList.get(i).activityInfo.packageName));
                Log.d(TAG, String.format("setDisplayList(): activieyInfo: %s " + mShareAppList.get(i).activityInfo.name));
        ...
        Collections.sort(mDisplayAppList, new ResolveInfo.DisplayNameComparator(mContext.getPackageManager()));
    }
